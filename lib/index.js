    ...
    .then(function(reply) {
        ...
        .then(function() {
            if(self.options.events){
                self.emit('delete', filename, file)
            }
            return reply.vclock
        })
...
RiakFs.prototype.readdir2 = function(_path, max, marker, callback) {
    var self = this;
    ...
    return Promise.all([
        ...
        self.riak.getIndexAll(searchParams)
    ])
...
RiakFs.prototype.mkdir = function(_path, mode, callback) {
    ...
    if(_path === '/'){
        return Promise.reject(new RiakFsError('EEXIST', 'Path already exists: /')).nodeify(callback)
    }
...
RiakFs.prototype.rmdir = function(_path, callback) {
    ...
    if(_path === '/'){
        return Promise.reject(new RiakFsError('EACCES', 'Can\'t remove /')).nodeify(callback)
    }
    ...
    .spread(function(reply, search) {
        ...
        if(self.options.events){
            p = p.then(function() {
                self.emit('delete', _path, reply.content[0].value)
            })
        }
            ...
            if(!file){ // create new file
                ...
                if(options.indexes){
                    options.indexes = options.indexes.map(function(ind) {
                        return {
                            key: self.options.root + '.files_custom_' + ind.key,
                            value: ind.value
                        }
                    })
                }
...
RiakFs.prototype.close = function(fd, callback) {
    ...
    if(self.options.events === true && (fd.modified || fd.isNew)){
        p = p.then(function() {
            self.emit(fd.isNew ? 'new' : 'change', fd.filename, fd.file)
        })
    }
...
RiakFs.prototype._updateMeta = function(filename, data, mergeFn) {
    var self = this;
