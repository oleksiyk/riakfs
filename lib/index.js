...
var RiakFs = function (options, riak) {
    ...
    self.riak = riak || new Client(options.riak);
...
RiakFs.prototype.read = function (fd, buffer, offset, length, position, callback) {
    ...
    p = Promise.resolve(); n = Math.floor(fd.position / Chunk.CHUNK_SIZE); oldPosition = fd.position;
...
RiakFs.prototype.write = function (fd, buffer, offset, length, position, callback) {
    ...
    p = Promise.resolve(); n = fd.position === 0 ? 0 : (Math.ceil(fd.position / Chunk.CHUNK_SIZE) - 1);
...
RiakFs.prototype.exists = function (_path, callback) {
    return this._checkShared(_path).spread(function (fs, __path) {
        if (__path === '/') {
            return true;
        }
