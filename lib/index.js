...
RiakFs.prototype.mkdir = function(__path, mode, callback) {
    ...
    function _mkdir () {
        return self._checkShared(__path).spread(function(fs, _path, readOnly) {
    ...
    if(typeof self.options.quotacheck === 'function' && !/^\/\.Trash(\/|$)/.test(__path)){
        return self.options.quotacheck()
        .then(function (ok) {
            if(ok === false){
...
RiakFs.prototype.close = function(fd, callback) {
    ...
    if(fd.fs.options.events === true && (fd.modified || fd.isNew)){
        p = p.then(function() {
            fd.fs.emit(fd.isNew ? 'new' : 'change', fd.filename, new Stats(fd.file, false));
        });
    }
...
RiakFs.prototype._rename = function(_old, _new) {
            ...
            if(file.share){ // source share, update destinations
                return Promise.map(file.share.to, function(to) {
                    return self.options.shared.fs(to.root).then(function(_tfs) {
                        var dstPath = '/Shared with Me/' + to.alias;
    ...
    .then(function () {
        if(self.options.events === true){
            if(/^\/\.Trash(\/|$)/.test(_new)){
                self.emit('delete', _old, new Stats(file, false));
            } else {
...
RiakFs.prototype.rename = function(_oldName, _newName, callback) {
    ...
    return Promise.all([
        ...
        if(oldName === '/' || oldName === '/Shared with Me' || oldName === '/.Trash'){
            throw new RiakFsError('EINVAL', 'Cannot rename / or /Shared with Me or /.Trash');
        }
        ...
        .spread(function(newFile, newDir, _oldFile) {
            ...
            if(oldFile.file.share && path.dirname(newName) !== '/Shared with Me' && ofs.options.root !== self.options.root){
...
RiakFs.prototype.setMeta = function(filename, data, callback) {
    return this.updateMeta2(filename, function(meta) {
        return _.partialRight(_.merge, _.defaults)(data || {}, meta || {});
    }).nodeify(callback);
};
...
RiakFs.prototype._checkShared = function(_path) {
    ...
    return new Promise(function(resolve) {
        var p;
        if(typeof self.options.shared.fs === 'function' && /^\/Shared with Me\//.test(_path)){
            p = _path.split('/');
            p = '/' + p[1] + '/' + p[2];
...
RiakFs.prototype.share = function(_path, root, alias, readOnly, callback) {
    var self = this, dstPath = '/Shared with Me/' + alias, p = Promise.resolve();
    ...
    if(_path === '/' || _path === '/Shared with Me'){
        return Promise.reject(new RiakFsError('EINVAL', 'Cannot share / or /Shared with Me')).nodeify(callback);
    }

    if(/^\/Shared with Me\/?/.test(_path)){
        var _p = _path.split('/');
        _p = '/' + _p[1] + '/' + _p[2];
        p = self.riak.get({
            bucket: self.filesBucket,
...
RiakFs.prototype.unshare = function(__path, root, callback) {
    ...
    __path = this._normalizePath(__path);
    ...
    return this._checkShared(__path).spread(function(fs, _path) {
        ...
        .then(function(reply) {
            ...
            if(dir.share && dir.share.to.length){
                ...
                __path = '/Shared with Me/' + dir.share.to[shareInd].alias;
