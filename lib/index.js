...
RiakFs.prototype.close = function (fd, callback) {
    ...
    if (fd.modified) {
        p = p.then(function () {
            // trying to catch extremely rare vclock issue where the following .put() call with vclock: fd.vclock was overwritten somehow
            return fd.fs.riak.get({
            ...
            .then(function (reply) {
                ...
                return fd.fs.riak.put({
                    bucket: fd.fs.filesBucket,
                    key: fd.filename,
                    vclock: reply ? reply.vclock : fd.vclock,
                    content: {
                        value: fd.file,
                        indexes: fd.indexes
                    },
