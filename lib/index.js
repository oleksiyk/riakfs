...
RiakFs.prototype._rename = function(_old, _new) {
    ...
    .then(function() { // update shares
        if(file && typeof self.options.sharedFs === 'function'){
            if(file.share){ // source share, update destinations
                return Promise.map(file.share.to, function(to) {
                    return self.options.sharedFs(to.root).then(function(_tfs) {
                ...
                return self.options.sharedFs(file._sharedFrom.root).then(function(_tfs) {
                    return _tfs.riak.get({
                        bucket: _tfs.filesBucket,
    ...
    return ofs.stat(from).then(function(stats) {
        if(stats.isFile()){
            ...
            return ofs.readdir(from).then(function(list) {
                return Promise.concurrencyLimit(list, 10, function(f) {
...
RiakFs.prototype.rename = function(_oldName, _newName, callback) {
    ...
    return Promise.all([
        ...
        if(oldName === '/' || oldName === '/Shared'){
            throw new RiakFsError('EINVAL', 'Cannot rename / or /Shared')
        }
        ...
        .then(function() {
            ...
            if(oldFile.file.share){ // rename of destination share dir was requested
                ofs = nfs;
                oldName = _oldName;
            }
...
RiakFs.prototype._checkShared = function(_path) {
    ...
    return new Promise(function(resolve) {
        var p;
        if(typeof self.options.sharedFs === 'function' && /^\/Shared\//.test(_path)){
            p = _path.split('/')
            p = '/' + p[1] + '/' + p[2]
...
RiakFs.prototype.unshare = function(__path, root, callback) {
    ...
    return this._checkShared(__path).spread(function(fs, _path) {
        if(typeof fs.options.sharedFs !== 'function'){
            throw new RiakFsError('EINVAL', 'options.sharedFs is not a function')
        }
        ...
        .then(function(reply) {
            ...
            if(dir.share && dir.share.to.length){
                ...
                return fs.riak.put({
                    ...
                    return fs.options.sharedFs(root)
                }).then(function(_tfs) {
                    return _tfs.riak.get({
                        bucket: _tfs.filesBucket,
                        key: dstPath
