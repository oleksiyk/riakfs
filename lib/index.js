...
mime.types['spin'] = 'application/json';
mime.types['view'] = 'application/json';
...
function makeNattempts(attempts, f, delay) {
    let attempt = 0;
    delay = delay || 2000;
    f = Promise.method(f);
        ...
        .then(function (reply) {
            ...
            if (fs.options.trash === true && !/^\/\.Trash(\/|$)/.test(filename)) {
                ...
                .then(function (_path) {
                    ...
                    .then(function () {
                        return fs._rename(filename, _path, options);
                    });
                });
            }
...
RiakFs.prototype.mkdir = function (__path, mode, callback) {
    ...
    if (typeof self.options.quotacheck === 'function' && !/^\/\.Trash(\/|$)/.test(__path)) {
        return self.options.quotacheck().then(function (ok) {
            if (ok === false) {
                throw new RiakFsError('EDQUOT', 'Storage quota exceeded');
            }
            return _mkdir();
        }).nodeify(callback);
    }
...
RiakFs.prototype.close = function (fd, callback) {
    ...
    if (fd.modified || fd.isNew) {
        p = p.then(function () {
            var _ct = _.get(fd, ['file', 'meta', 'contentType'], mime.lookup(fd.filename));

            fd.file.mtime = new Date().toISOString();
            fd.file.version += 1;

            if (_ct) { // overwrite mmmagic detected mimetype
                fd.file.contentType = _ct;
            }
...
RiakFs.prototype.rename = function (_oldName, _newName, callback) {
    ...
    return Promise.all([
        ...
        if (nfs.riak.usingFailover || ofs.riak.usingFailover) {
            throw new RiakFsError('EROFS', 'Read-only filesystem');
        }
        ...
        .then(function () {
            ...
            if (nfs.options.root === ofs.options.root) {
                p = nfs._rename(oldName, newName, options);
            } else {
    ...
    return self._readLink(__path).spread(function (fs, _path, readOnly) {
        if (_path === '/') {
            ...
            return Promise.all([
                ...
                .then(function (reply) {
                    return _.get(reply, ['content', 0, 'value'], {});
                })
            ]).spread(function (fsstats, stats) {
                return new Stats(_.defaults({
                    isDirectory: true,
                    ctime: new Date(0),
                    mtime: new Date(0),
                    stats: {
                        storage: fsstats.storage ? fsstats.storage.toNumber() : 0,
                        trashStorage: fsstats.trashStorage ? fsstats.trashStorage.toNumber() : 0,
                        files: fsstats.files ? fsstats.files.toNumber() : 0
                    }
                }, stats));
            });
        }
...
RiakFs.prototype.utimes = function (_path, atime, mtime, callback) {
    ...
    return self.open(_path, 'r').then(function (fd) { // don't change to 'w' here
...
RiakFs.prototype.makeTree = function (_path, callback) {
    ...
    return Promise.reduce(_path.split('/'), function (a, c) {
        ...
        p = a.join('/');
        if (p) {
            return self.mkdir(_.defaults({ filename: p }, options)).catch(RiakFsError, function (err) {
                if (err.code !== 'EEXIST') {
                    throw err;
                }
            })
...
RiakFs.prototype.rmTree = function (_path, callback) {
    ...
    function _rmTree(dir) {
        ...
        .then(function () {
            ...
            return self.rmdir(dir).catch({ code: 'ENOTEMPTY' }, () => {
                return _rmTree(dir).catch({ code: 'ENOENT' }, () => null); // this has possibility to go into inifinite loop
            });
        });
    }
    ...
    return self.stat(_path).then(function (stats) {
        ...
        return self.unlink(_.defaults({ filename: _path }, options));
    })
...
RiakFs.prototype.updateMeta2 = function (_filename, fn) {
    return this._readLink(_filename).spread(function (fs, filename, readOnly) {
        ...
        .then(function (reply) {
            if (filename !== '/' && (!reply || !reply.content)) {
                throw new RiakFsError('ENOENT', 'No such file or directory: ' + _filename);
            }
            file = _.get(reply, ['content', 0, 'value'], {});

            file.meta = fn(file.meta || {});

            // allow to overwrite contentType
            if (file.meta.contentType) {
                file.contentType = file.meta.contentType;
                delete file.meta.contentType;
            }

            return fs.riak.put({
                bucket: fs.filesBucket,
                key: filename,
                vclock: reply ? reply.vclock : undefined,
                content: {
                    value: file,
                    indexes: reply ? reply.content[0].indexes : undefined
                },
                type: fs.options.metaType
            });
        })
...
RiakFs.prototype.share = function (_path, root, alias, readOnly, callback) {
    ...
    .spread(function (sreply, _tfs) {
        ...
        if (self.riak.usingFailover || _tfs.riak.usingFailover) {
            throw new RiakFsError('EROFS', 'Read-only filesystem');
        }
    ...
    return self.open(filename, 'r+')
        .then(function (fd) {
            ...
            .then(function () {
                fd.file.size = size;
                fd.modified = true;
                return self.close(fd).return(size);
            });
        });
};
