...
RiakFs.prototype.mkdir = function(_path, mode, callback) {
    ...
    if (d !== '/') {
        p = self.riak.get({
            ...
            if(reply && reply.content && reply.content[0].value.isDirectory){
                return true
            }
            return false
        })
    ...
    ]).spread(function(reply, parentExists) {
        if(!parentExists){
    ...
    .spread(function(reply, parent) {
        ...
        if(/^(w|w\+|a|a\+|wx|wx\+|ax|ax\+)$/.test(flags)){
            ...
            if(file && /^(a|a\+)$/.test(flags)){
                var chunk = new Chunk(file, Math.floor(file.length/Chunk.CHUNK_SIZE), self)
...
RiakFs.prototype.rename = function(oldName, newName, callback) {
    ...
    function _rename(filename){
        ...
        .then(function(reply) {
            ...
            var newFilename = filename.replace(re, newName)
