        ...
        return self._checkShared(_filename).spread(function(fs, filename, readOnly) {
            ...
            .spread(function(reply, parent) {
                ...
                if(/^(w|w\+|a|a\+|wx|wx\+|ax|ax\+)$/.test(flags)){
                    var oldFile = file ? _.cloneDeep(file) : null;
                    if(readOnly || (fs.options.trash === true && /^\/\.Trash\//.test(filename))){
                        throw new RiakFsError('EACCES', 'Permission denied: ' + _filename);
                    }
...
RiakFs.prototype.read = function(fd, buffer, offset, length, position, callback) {
    ...
    var p = Promise.resolve(), n = Math.floor(fd.position/Chunk.CHUNK_SIZE), oldPosition = fd.position;
...
RiakFs.prototype.rmTree = function (_path, callback) {
    ...
    function _rmTree (dir){
        return self.readdir(dir).then(function (files) {
            return Promise.concurrencyLimit(files, 10, function (f) {
...
RiakFs.prototype.share = function(_path, root, alias, readOnly, callback) {
    var self = this, dstPath = '/Shared/' + alias, p = Promise.resolve();
