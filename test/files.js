...
describe('Files', function () {
            ...
            it('should create new file wih flags=' + flag, function () {
                return riakfs.open('/testnewfile_' + flag, flag)
                    .then(function (fd) {
                        ...
                        new Date(fd.file.mtime).getTime().should.be.closeTo(new Date().getTime(), 500);
                        fd.file.should.have.property('ctime');
                        new Date(fd.file.ctime).getTime().should.be.closeTo(new Date().getTime(), 500);
            ...
            return riakfs.open('/testWriteFile', 'w').then(function (fd) {
                return riakfs.write(fd, 'test', 0, 4, null).then(function () {
                    return riakfs.close(fd).then(function () {
                        ...
                        return riakfs.open('/testWriteFile', 'r').then(function (_fd) {
                            var buffer = new Buffer(_fd.file.size);
                            return riakfs.read(_fd, buffer, 0, _fd.file.size).then(function (length) {
                                length.should.be.eql(_fd.file.size);
            ...
            return riakfs.open('/testWriteFileUtf8', 'w').then(function (fd) {
                return riakfs.write(fd, 'тест', 0, 8, null).then(function () {
                    return riakfs.close(fd).then(function () {
                        ...
                        return riakfs.open('/testWriteFileUtf8', 'r').then(function (_fd) {
                            var buffer = new Buffer(_fd.file.size);
                            return riakfs.read(_fd, buffer, 0, _fd.file.size).then(function (length) {
                                length.should.be.eql(_fd.file.size);
            ...
            return riakfs.open('/тестФайл', 'w').then(function (fd) {
                return riakfs.write(fd, 'тест', 0, 8, null).then(function () {
                    return riakfs.close(fd).then(function () {
                        ...
                        return riakfs.open('/тестФайл', 'r').then(function (_fd) {
                            var buffer = new Buffer(_fd.file.size);
                            return riakfs.read(_fd, buffer, 0, _fd.file.size).then(function (length) {
                                length.should.be.eql(_fd.file.size);
                                buffer.slice(0, length).toString().should.be.eql('тест');
    ...
    describe('#read', function () {
        ...
        it('should read data incrementaly', function () {
            var fd;
            return riakfs.open('/testReadFile', 'w').then(function (_fd) {
                return riakfs.write(_fd, 'test', 0, 4, null).then(function () {
                    return riakfs.close(_fd);
                });
            })
            .then(function () {
                return riakfs.open('/testReadFile', 'r');
    ...
    describe('#writefile', function () {
        testfiles.forEach(function (f) {
            it('should create and write file (' + path.basename(f.path) + ')', function () {
